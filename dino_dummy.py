...
def main():
    with mss() as screenshotter:
        ...
        print(landscape)
        if landscape:
            pyautogui.moveTo(landscape['left'], landscape['top'])
            time.sleep(0.25)
            pyautogui.moveTo(landscape['left'] + landscape['width'], landscape['top'])
            time.sleep(0.25)
            pyautogui.moveTo(landscape['left'] + landscape['width'], landscape['top'] + landscape['height'])
            time.sleep(0.25)
            ...
            while True:
                ...
                if distance < last_distance:
                    ...
                    if np.where(res >= 0.7)[0]:
                        print('\nGAME OVER')
                        notify_gameover(score)
                        pyautogui.hotkey('ctrl', 'r')
                        pyautogui.hotkey('alt', 'tab')
                        image[y1:y2, x1:x2] = 127
                        plt.imshow(image, 'gray')
                        plt.show()
                        break
                last_distance = distance

                command = get_command(distance, size, speed)
                if command:
                ...
                print('Distance: {:3d} Size {:2d} Speed: {} Score: {:.0f}'.format(distance, size, speed, score) + ' ' * 10, end='\r')
                elapsed = time.time() - start
                wait_for = 1. / 60. - elapsed
                if wait_for > 0:
                    time.sleep(wait_for)
...
def get_command(distance, size, speed):
    if distance < 85:
        return 'up'
    else:
        return ''
